...
namespace Core.Contracts.EZCRUD
{
    ...
    [Produces("application/json")]
    [Consumes("application/json")]
    public class EZCRUDEnpoint<C, U, D, Object, Id, Service> : Controller where Id : IComparable where U : IConvertible<Object> where Service : IEZCRUDService<C, Object, Id>
    {
        private Func<D, Id> DeleteIdSelector { get; set; } = null!;

        public EZCRUDEnpoint(Func<D, Id> deleteIdSelector)
        {
            DeleteIdSelector = deleteIdSelector;
        }
        ...
        [HttpPut]
        public virtual async Task<IActionResult> Update([FromBody] U model, [FromServices] Service service)
        {
            if (model == null)
            {
                return BadRequest();
